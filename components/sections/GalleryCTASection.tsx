...
export default function GalleryCTASection() {
  return (
    <>
      <section id="gallery" className="border-t border-white/10 bg-[#0a0a0f] py-20">
        <div className="mx-auto grid max-w-7xl gap-10 px-6 sm:px-10 lg:grid-cols-2 lg:items-center">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.3em] text-[var(--color-gold)]">Gallery</p>
            ...
            <div className="mt-6 grid grid-cols-3 gap-2">
              {Array.from({ length: 6 }).map((_, i) => (
                <div key={i} className="grid aspect-[4/3] place-items-center rounded-lg border border-white/10 bg-gradient-to-br from-[#16202b] to-[#0b0f14] text-white/15">
                  <ImageIcon size={22} />
                </div>
              ))}
            </div>
            <a href="/community" className="mt-6 inline-flex items-center gap-2 rounded-md border border-[var(--color-gold)]/50 px-5 py-3 text-[11px] font-bold uppercase tracking-[0.18em] text-[var(--color-gold)] transition hover:bg-[var(--color-gold)]/10">
              View Gallery <ArrowRight size={15} />
            </a>
          </div>

          <div>
            <p className="text-xs font-bold uppercase tracking-[0.3em] text-[var(--color-gold)]">What Members Say</p>
            <Quote className="mt-6 text-[var(--color-gold)]" size={40} />
      ...
      {/* Closing CTA */}
      <section className="relative overflow-hidden border-t border-white/10 bg-gradient-to-b from-[#0d0d14] to-[#0a0a0f] py-20 text-center">
        <p className="text-xs font-bold uppercase tracking-[0.3em] text-[var(--color-gold)]">Ready to be part of RCC?</p>
